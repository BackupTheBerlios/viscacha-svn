	...
	function createFeed() {
		...
		if ($this->image!=null) {
			$imageStr = "<a href='".$this->image->link."'".$targetInsert.">".
							...
							FeedCreator::iTrunc($this->htmlspecialchars($this->image->title),100).
		...
		if ($this->title) {
			$feedArray[] = "<div class='".$this->stylePrefix."title'><a href='".$this->link."' ".$targetInsert." class='".$this->stylePrefix."title'>".
				FeedCreator::iTrunc($this->htmlspecialchars($this->title),100)."</a></div>";
		}
			...
			if ($this->items[$i]->title) {
				if ($this->items[$i]->link) {
					$feedArray[] =
						...
						"item_title'".$targetInsert.">".FeedCreator::iTrunc($this->htmlspecialchars(strip_tags($this->items[$i]->title)),100).
						"</a></div>";
				} else {
					$feedArray[] =
						"<div class='".$this->stylePrefix."item_title'>".
						FeedCreator::iTrunc($this->htmlspecialchars(strip_tags($this->items[$i]->title)),100).
						"</div>";
				}
			}
