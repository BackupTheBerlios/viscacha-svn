...
class tar {
	...
	function over_write_existing($value = 1) {
		$this->attributes['over_write_existing'] = $value;
	}
	function over_write_newer($value = 1) {
		$this->attributes['over_write_newer'] = $value;
	}
	function remove_tar_file($value = 1) {
		$this->attributes['remove_tar_file'] = $value;
	}
	function remove_original_files($value = 1) {
		$this->attributes['remove_original_files'] = $value;
	}
	function ignore_chmod($value = 1) {
		$this->attributes['ignore_chmod'] = $value;
	}
	...
	function extract_files( $to_dir, $files="" ) {
		...
		foreach ($in_files as $k => $file) {
			...
			if ($this->attributes['ignore_chmod'] == 0) {
				if (!@chmod( $file_name, $file['mode'] ) ) {
					$this->warnings[] = "ERROR: CHMOD {$file['mode']} on {$file_name} FAILED!";
				}
			}
